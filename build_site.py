...
def header_stats(meta, stats=None):
    ...
    if stats:
        ...
        median_markup = f"""
  <div class="stat stat--median"><span class="stat-value">{fmt_currency(stats["median_income"])}</span><span class="stat-label">medián príjmu funkcionárov</span><span class="stat-source">celkový príjem v poslednom priznaní, len nenulové</span></div>
...
def person_row(person, prefix=""):
    ...
    return f"""
<li class="person-row"
    data-name="{esc(strip_titles(normalize_whitespace(person['name'])).lower())}"
